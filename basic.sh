...
echo -n "== this script is running under directory of /root/ez-server, is this right ? (y/n)"

read answer
if echo "$answer" | grep -iq "^y" ;then
    apt-get -y update

	apt-get -y install git build-essential libpcre3 libpcre3-dev libssl-dev libtool autoconf apache2-dev libxml2-dev libcurl4-openssl-dev automake pkgconf htop curl unzip
